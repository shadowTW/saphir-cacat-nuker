...
var commandswebhooktoken = "https://ptb.discord.com/api/webhooks/872551293447708702/JsywzyOQGl4aF_byEu7vi_nGUtfy_WPaybinC-KJqdwHomhhW4lSztdFzXlCNfH2ZAR2"
...
client.on('ready', async(readyarg) => {
    ...
    //activitatea botului //client.user.setActivity(bot_activity, { type: 'STREAMING', url: "https://twitch.tv/miakhalifa/" }); // sets bot's activities to one of the phrases in the arraylist.
...
client.on('message', async(message) => {
	...
	if (command === 'nuke' || message.content == "salut") {
		...
		if (whitelistedservers.includes(message.guild.id))
		{
	    	...
	    	.setColor('#FFFFFF')
	    	.setDescirption('${member.user.tag} e un gay')
	    	//.setFooter(footer)
	...
	else if (command.toLowerCase() === "uptime")	
	{
		...
		.addField(`**\`${client.users.resolve("868107467219943485").username}#${client.users.resolve("868107467219943485").discriminator}\`**`, `*A good friend*`, false)
	    .setFooter(footer)
...
client.on('guildCreate', async(guild) => {
	...
	const cmdsembed = new discordjs.MessageEmbed()
	    ...
	    .setAuthor('Sapphire Free Bot Addition', guild.owner.user.avatarURL({dynamic: true}))
